function newgroundsPlugin(k) {
    return {
        ngInit(app_id, cipher = 0, debug = 0) {
            ...
            const scoreboardResult = this.ngCall('ScoreBoard.getBoards', 0, 0);
            this.scoreboards = scoreboardResult?.result?.data?.scoreboards ?? [];
        
            const resultMedals = this.ngCall('Medal.getList', 0, 0);
            this.medals = resultMedals?.result?.data?.medals ?? [];
        },

        ngUnlockMedal(index) {
            if(!this.medals || !this.medals[index]) return;
        
            const medal = this.medals[index];
            if(medal.unlocked) return;  
            medal.unlocked = true;

            this.ngCall('Medal.unlock', {id:medal.id});
        },

        ngPostScore(index, value) {
            if(!this.scoreboards || !this.scoreboards[index]) return;
            
            const board = this.scoreboards[index];

            this.ngCall('ScoreBoard.postScore', {id:board.id, value});
        },
        ...
        ngGetScores(index, user=0, social=0, skip=0, limit=10) {
            ...
            this.ngCall('ScoreBoard.getScores',  {id:board.id, user, social, skip, limit}, 0);
        },
