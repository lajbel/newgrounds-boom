function newgroundsPlugin(k) {
    const newgrounds = {
        ngInit(app_id, cipher = 0, debug = 0) {
            ...
            k.session_id = url.searchParams.get('ngio_session_id') ?? 0;
        
            const scoreboardResult = ngCall('ScoreBoard.getBoards', 0, 0);
            k.scoreboards = scoreboardResult?.result?.data?.scoreboards ?? [];
        ...
        ngCall(component, parameters=0, async=1) {
            ...
            xmlHttp.open('POST', url, k.ngDebug? 0 : async);
            xmlHttp.send(formData);
        
            if(xmlHttp.responseText) {
                if(k.ngDebug) console.log(xmlHttp.responseText);
                k.responseText = xmlHttp.responseText;
        ...
        ngEncryptCall(call) {
            if(!k.cipher) return call;
        
            const aesKey = k.CryptoJS.enc.Base64.parse(k.cipher);
            const iv = k.CryptoJS.lib.WordArray.random(16);
            const encrypted = k.CryptoJS.AES.encrypt(JSON.stringify(call), aesKey, {iv});
            const secure = k.CryptoJS.enc.Base64.stringify(iv.concat(encrypted.ciphertext));
